...
// Normalise id — MongoDB products use _id, local products use id
const pid = (item) => String(item._id ?? item.id);
    ...
    <main className="max-w-7xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100">
          Your Cart{' '}
          <span className="text-gray-400 dark:text-gray-500 text-lg font-normal">({totalItems} items)</span>
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => (
            <div key={pid(item)}
              className="flex gap-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 shadow-sm">
              ...
              <div className="flex-1 flex flex-col gap-1">
                ...
                <div className="flex items-center gap-3 mt-auto">
                  ...
                  <div className="flex items-center border border-gray-300 dark:border-gray-600 rounded-full overflow-hidden">
                    <button
                      onClick={() => updateQuantity(pid(item), item.quantity - 1)}
                      className="px-3 py-1 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                      aria-label="Decrease quantity"
                    >−</button>
                    <span className="px-3 py-1 text-sm font-medium min-w-[2rem] text-center text-gray-800 dark:text-gray-100">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => updateQuantity(pid(item), item.quantity + 1)}
                  ...
                  {/* Remove */}
                  <button
                    onClick={() => removeFromCart(pid(item))}
        ...
        <aside className="lg:sticky lg:top-20 h-fit bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
          ...
          <div className="space-y-3 text-sm mb-6">
            {items.map((item) => (
              <div key={pid(item)} className="flex justify-between text-gray-600 dark:text-gray-400">
